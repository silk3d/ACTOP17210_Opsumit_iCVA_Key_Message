    ...
    .factory('InputManager', ['$http','$rootScope', '$state', "$log", "VeevaMessaging", function($http, $rootScope, $state, $log, VeevaMessaging){
        ...
        return{
            ...
            handleGesture:function( target_state, gesture, controllerScope, sourceFcn ) {
                        ...
                        if( String(gesture).toLowerCase() == "swipeleft" ) {
                            
                        } else if( String(gesture).toLowerCase() == "swiperight" ) {
...
.run(
    ['$rootScope', '$state', '$stateParams', '$localStorage', 'HotspotsManager', 'VeevaMessaging', '$log', 'APP_VERSION', '$localStorage', 'InputManager',
        function($rootScope, $state, $stateParams, $localStorage, HotspotsManager, VeevaMessaging, $log, APP_VERSION, $localStorage,  InputManager ) {
                ...
                controller: ['$rootScope', '$scope', '$state', 'HotspotsManager', '$log', 'VeevaMessaging', '$localStorage', 'KEY_MESSAGE_ID',
                    function($rootScope, $scope, $state, HotspotsManager, $log, VeevaMessaging, $localStorage, KEY_MESSAGE_ID) {
                        ...
                        /********  gesture code begin ******/
								    ...
								    if ( !$localStorage.start_state ){
										$localStorage.start_state = "viewTemplate.home";
									}	
                     
                                    setTimeout(function () {
											VeevaMessaging.sendMessage($localStorage.start_state);
                                    }, 250);     
